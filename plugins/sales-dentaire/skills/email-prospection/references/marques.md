# Marques du portefeuille — positionnement

**Statut : incomplet.** Cette liste reprend les marques dictées par Bruce
lors de la construction de ce skill, mais leur positionnement (ce qui les
différencie, pour quel besoin les mettre en avant) n'a pas été fourni — je
ne l'invente pas, un argumentaire de marque erroné envoyé à un vrai prospect
coûterait plus cher qu'une case vide. À compléter avec Bruce, marque par
marque, au fur et à mesure.

**Point à trancher avec Bruce :** PASSATION.md et CLAUDE.md mentionnent
« 18 marques » de façon constante, mais la liste dictée compte 23 noms
distincts une fois le doublon « MHC » retiré. Écart toujours ouvert — deux
indices trouvés dans le dépôt (`docs/superpowers/specs/2026-08-25-catalogue-produits-pilote-planmeca-design.md`),
sans trancher le compte :
- La convention de nommage réelle laisse tomber « Cefla » — la spec et la
  table `produits` disent « Anthos » et (attendu) « Castellini », pas
  « Cefla Anthos »/« Cefla Castellini ». Renommé ci-dessous en conséquence.
- La spec cite **Acteon** comme une des 18 marques du catalogue — absente
  de la liste dictée pour ce skill. Oubli à la dictée, ou marque
  volontairement exclue de la prospection (pas du catalogue) ? À confirmer.
- Bruce a signalé trois oublis probables supplémentaires à la dictée
  initiale : **Mectron, Carestream, Vatech** — ajoutées ci-dessous comme
  Acteon, à confirmer plutôt que tenues pour certaines.

Je n'ai pas pu lire directement `Configurateur/` dans iCloud Drive (accès
refusé par macOS, piège déjà documenté dans PASSATION.md) — si tu donnes
l'accès ou listes le dossier toi-même, la liste des sous-dossiers donnerait
le compte exact.

| Marque | Catégorie de produit | Positionnement | Angle d'approche |
|---|---|---|---|
| Planmeca | — | *à compléter* | — |
| Anthos | — | *à compléter* | — |
| Castellini | — | *à compléter* | — |
| Acteon | — | *présente dans la spec catalogue, absente de la liste dictée — à confirmer* | — |
| Mectron | — | *oubli probable signalé par Bruce — à confirmer* | — |
| Carestream | — | *oubli probable signalé par Bruce — à confirmer* | — |
| Vatech | — | *oubli probable signalé par Bruce — à confirmer* | — |
| OMS | — | *à compléter* | — |
| DCE | — | *à compléter* | — |
| DCI | — | *à compléter* | — |
| DBI | — | *à compléter* | — |
| Cattani | — | *à compléter* | — |
| MHC | — | *à compléter* | — |
| Euronda | — | *à compléter* | — |
| Dental Art | — | *à compléter* | — |
| Rossicaws | Mobilier métallique | Même usine que Dental Art, marque distincte | *à compléter* |
| Astra | — | *à compléter* | — |
| Bien Air | — | *à compléter* | — |
| Melag | — | *à compléter* | — |
| NSK | — | *à compléter* | — |
| Dürr Dental | — | *à compléter* | — |
| Zilfor | — | *à compléter* | — |
| Gamasonic | — | *à compléter* | — |
| IPP Pharma | — | *à compléter* | — |
| Oudin | — | *à compléter* | — |
| Faro | — | *à compléter* | — |
| Zénium | — | *à compléter* | — |

Une fois une marque renseignée, l'e-mail généré par `email-prospection` peut
citer sa catégorie et son positionnement réel plutôt qu'un argumentaire
générique du type « équipement de qualité » — le but de ce fichier est
justement d'éviter ça.
