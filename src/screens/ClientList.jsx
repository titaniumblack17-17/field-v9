...
export default function ClientList({ onBack, onSelect, onCreate }) {
  ...
  return (
    <div className="min-h-screen bg-fond overflow-x-hidden">
      ...
      <header className="sticky top-0 z-10 bg-fond/90 backdrop-blur px-4 pt-6 pb-3 overflow-x-hidden">
        {/* Écran secondaire depuis le passage de Brief soir en écran de
            démarrage : plus de pilules de nav ici (Brief/Pipeline/Capture
            vivent désormais sur Brief), un simple retour suffit — même
            gabarit de bouton que Pipeline.jsx/Capture.jsx. */}
        <div className="flex items-center gap-2 mb-3">
          ...
          {/* Action principale de l'écran : dans la variante accent-vif
              (cyan, choisi hors de la famille violet/bleu pour trancher)
              plutôt que l'accent générique, pour se voir en premier. Texte
              foncé plutôt que blanc : le cyan est trop clair pour un bon
              contraste avec du blanc. */}
          <button
            onClick={onCreate}
            aria-label="Nouveau client"
            className="w-12 h-12 rounded-full bg-accent-vif text-[#0A2E33] text-2xl leading-none flex items-center justify-center shadow flex-shrink-0"
          >
            +
          </button>
        </div>
